...
class PortfolioGUI:
    def __init__(self, root):
        ...
        # XBTSX.STH Tab
        self.xbtsx_sth_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.xbtsx_sth_frame, text="XBTSX.STH Portfolio")
        self.xbtsx_sth_tree = self.create_treeview(self.xbtsx_sth_frame)
        ...
        self.xbtsx_sth_total_label = ttk.Label(footer_frame, text="XBTSX.STH Total: $0.00", font=("Helvetica", 10))
        self.xbtsx_sth_total_label.grid(row=0, column=2, sticky=tk.W)
    ...
    def run_valuation(self):
        try:
            ...
            # Fetch XBTSX.STH Price
            try:
                with open("config_xbtsx_sth.json", "r") as f:
                    sth_config = json.load(f)
                    prices["XBTSX.STH"] = valuation.get_xbtsx_sth_price_usd(sth_config)
            except Exception as e:
                print(f"XBTSX.STH Price error: {e}")
            ...
            for p in valuation.PORTFOLIOS:
                ...
                if relevant_val is not None:
                    ...
                    elif p["name"] == "XBTSX.STH":
                        xbtsx_sth_total = relevant_val
                        self.update_tree_generic(self.xbtsx_sth_tree, details, "standard")
